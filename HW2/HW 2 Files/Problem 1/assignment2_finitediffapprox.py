...
#Absolute errors
EdPbydt_frwd  =  np.abs(VdPbydt_frwd - dPbydr_analy(rw))  #errors in forward difference E stands for vectorized errors
EdPbydt_bkwrd = np.abs(VdPbydt_bkwrd - dPbydr_analy(rw))  #errors in backward difference E stands for vectorized errors
EdPbydt_cntr  =  np.abs(VdPbydt_cntr - dPbydr_analy(rw))  #errors in centered difference E stands for vectorized errors
...
plot = plt.loglog(dt,power_law(dt, *ZdPbydt_frwd),'r--',label=r'$fit: O( \Deltat^{%5.3f})$' %ZdPbydt_frwd[1] )
