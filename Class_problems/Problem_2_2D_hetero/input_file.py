...
def inputfile(fluid,reservoir,numerical,BC,IC):
    ...
    reservoir.W  = 8000.0  #width of the reservoir [feet]    
    ...
    numerical.dx = (reservoir.L/numerical.Nx)*np.ones((numerical.N, 1)) #block thickness in x vector
    numerical.dy = (reservoir.W/numerical.Ny)*np.ones((numerical.N, 1)) #block thickness in y vector
    ...
    for i in range(1,numerical.Ny):
        numerical.yc[i,0] = numerical.yc[i-1,0] + 0.5*(numerical.dy[i-1,0] + numerical.dy[i,0])
    ...
    BC.type  = [['Dirichlet'],['Neumann'],['Neumann'],['Neumann']] #type of BC: left, right, bottom, top
    BC.value = [[2000],[0],[0],[0]]    #value of the boundary condition: psi or ft^3/day
