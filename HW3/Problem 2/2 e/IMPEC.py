...
while (t_D[k] < numerical.PVI):      
    ...
    #restricting the PVI according to the problem
    if t_D[k] > 0.05:
...
'''
'''
np.savez('tvsC_N30X30',t_D[0:k-1],Cexit[0:k-1])

plt.figure()
plt.plot(t_D[0:k-1],Cexit[0:k-1])
plt.xlabel(r'$t_D$')
plt.ylabel(r'$C_D$')
plt.savefig('CdvsTd2e.png',bbox_inches='tight', dpi = 600)
'''

initial_plot(reservoir,numerical,C_plot[:,1154],t_D[1154])
initial_plot(reservoir,numerical,C_plot[:,5762],t_D[5762])
initial_plot(reservoir,numerical,C_plot[:,11522],t_D[11522])