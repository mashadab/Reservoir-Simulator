...
def initial_plot(reservoir,numerical,P,time):
    ...
    plt.title('Concentration %0.1f [days]' %time)
    plt.axis('scaled')
    plt.xlabel(r'$x [feet]$')
    plt.ylabel(r'$y [feet] $')
    clb = plt.colorbar(ax1)
    clb.set_label(r'$C$', labelpad=-40, y=1.1, rotation=0)
    plt.tight_layout(pad=0.4, w_pad=0.5, h_pad=1.0)
    plt.savefig(f'concentration_{time}days.png',bbox_inches='tight', dpi = 600)
